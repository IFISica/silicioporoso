...
from EffectiveIndexModels import looyenga, bruggeman, maxwell_garnett, lorentz_lorenz
from RefractiveIndexDB import silicon, air

class BeamParameters():
    '''
        Build a class for the parameters of the beam.

            beam = BeamParameters(wavelength, angle_incidence=0.0, polarisation=0.5)

        wavelength: in nanometers generally
        angle_incidence: in degrees, default is 0.0
        polarisation: value between [0, 1], default is 0.5 (unpolarised light)
                      1 (p-TM), 0 (s-TE), or any mix intermediate
    '''

    def __init__(self, wavelength, angle_incidence=0.0, polarisation=0.5) -> None:  
        assert(0.0 <= polarisation <= 1.0), "the polarisation should be between 0 and 1"
        if isinstance(wavelength, int):
            assert(wavelength > 0.0), "the wavelength should be > 0"
        elif isinstance(wavelength, numpy.ndarray):
            assert(wavelength.all() > 0.0), "the wavelength should be > 0"
        assert(0.0 <= angle_incidence <= 90.0), "the angle of incidence should be between 0 and 90 degrees"  
